...
| **IDC 市场分析** | IDC 第三方市场调研数据 | `idc_printer_market`、`idc_pivot_templates` 等 | 已实现，11 个接口 |
...
| 11 | `/api/idc/explore-v2` | POST | 透视分析 V2（范围周期筛选） | P0 |
...
### 11.13 透视分析 V2（范围周期筛选）

```
POST /api/idc/explore-v2
```

**功能**：与 `/api/idc/explore` 核心能力一致，区别在于时间周期筛选方式不同。

**适用场景**：需要跨越多个半年度进行聚合分析的场景，如同比计算、跨期趋势对比、多国多周期联动等。
...
```json
{
  ...
  "valueFields": [
    { "aggregation": "sum_units", "format": "number", "decimalPlaces": 0 },
    { "aggregation": "asp", "format": "currency", "decimalPlaces": 2 }
  ],
  "sortField": "sum_units",
  "sortOrder": "desc",
  "page": 1,
...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| `filters` | object | 否 | null | 筛选条件，同 `/explore`，支持 `country`、`half_year`、`half_years`、`brand` 等 |
| `product_type` | string | 否 | `"all"` | 品类：`all`=全品类，`laser`=激光，`inkjet`=喷墨 |
| `startPeriod` | string | 否 | null | 起始半年度，格式 `2025H1`。不填时表示从最早周期开始 |
| `endPeriod` | string | 否 | null | 结束半年度，格式 `2025H2`。不填时表示到最新周期结束 |
| `rowFields` | string[] | 是 | - | 行维度列表，最多 3 个 |
| `colField` | string | 否 | null | 列维度 |
| `valueFields` | object[] | 是 | - | 值字段配置列表，最多 5 个 |
| `sortField` | string | 否 | `"sum_units"` | 排序字段 |
| `sortOrder` | string | 否 | `"desc"` | 排序方向：`asc` / `desc` |
| `page` | int | 否 | `1` | 页码 |
| `pageSize` | int | 否 | `20` | 每页条数，最大 200 |

**时间筛选逻辑**：

| `startPeriod` | `endPeriod` | SQL 行为 |
|---|---|---|
| 有值 | 有值 | `half_year >= '2025H1' AND half_year <= '2025H2'`（范围） |
| 有值 | 无值 | `half_year = '2025H1'`（单期） |
| 无值 | 有值 | `half_year = '2025H2'`（单期） |
| 无值 | 无值 | 无周期过滤（返回全量数据） |
...
| `all` | 无 product 过滤 |
| `laser` | `product = 'Laser'` |
| `inkjet` | `product = 'Inkjet'` |

**统计量映射**：与 `/api/idc/explore` 完全一致，参见 11.12 节。

**响应结构**：与 `/api/idc/explore` 完全一致。
...
POST /api/idc/explore-v2
{
  ...
  "filters": { "country": ["US"] },
  "rowFields": ["Half Year"],
  "valueFields": [{ "aggregation": "sum_units" }]
}
