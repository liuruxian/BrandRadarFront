<template>
  <div class="page-container">
    ...
    <div class="task-layout">
      <div class="card trigger-panel animate-fade-up">
        ...
        <div class="form-group">
          <label class="toggle-row">
            <div
              class="cyber-toggle"
              :class="{active:form.update_existing}"
              @click="form.update_existing=!form.update_existing"
            >
          ...
          >
            <div
              class="cyber-toggle"
              :class="{active:form.force}"
              @click="form.force=!form.force"
            >
              <div class="toggle-track"><div class="toggle-thumb" /></div>
            </div>
      ...
      <div
        class="task-list-panel animate-fade-up"
      ...
      >
        <div v-if="loading&&tasks.length===0">
          ...
          >
            <div
              class="skeleton"
              style="height:14px;width:40%;margin-bottom:8px"
            />
            <div
              class="skeleton"
              style="height:11px;width:60%"
            />
          </div>
        </div>
        <div
        ...
        >
          ...
          >
            <div class="task-head">
              ...
              <div class="task-info">
                <div class="task-name">
                  {{ task.brand||'全部品牌' }}<span
                    v-if="task.country"
                    class="task-country"
                  > · {{ task.country }}</span>
                </div>
                <div class="task-id font-mono">
            ...
            <div
              v-if="task.status==='done'&&task.product_count!==null"
              class="task-stats"
            >
...
.task-running { border-color:rgba(245,158,11,0.35); animation:borderFlow 2s ease-in-out infinite; }
.task-done    { border-color:rgba(16,185,129,0.25); }
.task-failed  { border-color:rgba(244,63,94,0.3); }
